...
export MAKE_ROOT := $(shell git rev-parse --show-toplevel)

# Absolute path to the resource directory for make.
export MAKE_RESOURCE_ROOT := $(MAKE_ROOT)/resources/make

# Absolute path to the directory containing the files for this framework.
export MAKE_RUNNER_ROOT := $(MAKE_RESOURCE_ROOT)/runner

# Make sure that we have a consistent locale handling across systems.
export LC_ALL := C

# Loads the user specific configuration.
include $(MAKE_RESOURCE_ROOT)/config.mk
...
export BASH_ENV := $(MAKE_RUNNER_ROOT)/env.bash '$(BASH_ENV)'
...
#
# - `bin` contains global scripts that can be invoked directly or via make.
# - `resources/make/bin` contains make specific user scripts. This is where
#   scripts for custom goals that depend on the abstractions of the make-runner
#   framework should be placed.
#
# Note the order of the paths which allows users to customize individual scripts
# by providing their own version.
export PATH := $(MAKE_ROOT)/bin:$(MAKE_RESOURCE_ROOT)/bin:$(MAKE_RUNNER_ROOT)/bin:$(PATH)
...
.PHONY: help
help: ## display all available make goals and exit
	make-runner-help $(MAKEFILE_LIST)

.PHONY: env
env: ## display make’s environment and exit
	printenv | sort

.PHONY: bootstrap-system
bootstrap-system: ## install recommended software
	$@

.PHONY: runner-update
runner-update: ## update the make-runner installation [version: latest]
	$@
